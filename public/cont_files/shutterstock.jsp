...
if (typeof usi_analytics === 'undefined') {
	usi_analytics = {
		...
		usi_app.main = function () {
			...
			usi_app.is_suppressed = usi_app.is_confirmation_page || !usi_app.is_english || location.href.indexOf("qa.shutterstock.com") != -1; /*|| !usi_app.is_video()*/;
			...
			if (usi_app.coupon != "" && usi_app.is_checkout_page) {
				if (!usi_date.is_date_within_range(usi_app.now, '2021-2-22 0:01', '2021-3-10 23:59')) {
		...
		usi_app.listen_for_active_cart = function() {
			...
			try {
				if (usi_app.is_cart_page) {
					...
					if (document.querySelector("[aria-label='Shopping cart']") != null) {
						if (document.querySelector("[aria-label='Shopping cart']").textContent != "") {
							usi_cookies.set("usi_active_cart", "true", 24*60*60, true);
		...
		usi_app.load = function () {
			...
			if (!usi_app.is_existing_customer && usi_app.is_footage_in_cart && !usi_date.is_between('2021-05-12T09:00', '2021-06-03T20:59:59')) {
				// 10% off creative refresh (updated 11/2020)
				usi_commons.log('[ load ] * * * Load 10% Off Creative Refresh TT * * *');
				usi_commons.load_view("jC4DtZvR3vp1ibcrSKJbf4L", "32945", usi_commons.device);
			} else if (!usi_app.is_existing_customer && usi_app.is_footage_in_cart && usi_date.is_between('2021-05-12T09:00', '2021-06-03T20:59:59')) {
				// Sitewide events
				usi_commons.log('[ load ] * * * Load Sitewide Events TT * * *');
		...
		usi_app.is_image = function() {
			try {
				if (location.href.indexOf("/images") != -1 || location.href.indexOf("/image-photo/") != -1) return true;
				for (var i=0; i<dataLayer.length; i++) {
		...
		usi_app.interval = function() {
			if (typeof(usi_app.interval_id) === "undefined") {
				usi_app.interval_id = setInterval(function() {
					if (usi_date.is_date_before(usi_app.today, '2020-11-09 21:00') && usi_app.is_image() && location.href.indexOf("/purchase") != -1) {
						usi_split_test.instantiate('32649', usi_app.force_group);
						if (usi_split_test.get_group('32649') == '1') {
							// 20% off images
							usi_commons.load_view("Uiq4rNqJzLlcInncBzQficc", "32647", usi_commons.device);
						}
					}
				} , 1000);
			}
			...
			try {
				if (usi_commons.device != "mobile") {
					...
					if (usi_app.suppress_app) {
						...
						setInterval(usi_app.check_for_change, 500);
					}
				}
			} catch (err) {
				usi_commons.report_error(err);
